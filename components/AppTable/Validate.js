		...
		switch (field.type) {
			...
			case "email":
				...
				if (checkEmailValue(value)) {
					return createError(error, "Введите корректный email");
				}
				break;
			// Проверка пароля
			case "password":
...
// Проверка значения на email
const checkEmailValue = email => {
	const pattern = /^(|(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6})$/i;
