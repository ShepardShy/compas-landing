<template>
	<td
		...
		:class="[`table__item_${props.item.type}`, props.item.fixed ? 'table__item_fixed' : '', !props.item.enabled ? 'table__item_hidden' : '', props.item.isUpdated ? 'table__item_updated' : '']"
		@click="event => doubleClick(event, props.row)"
	>
		<div
			class="table-item__content"
			:class="`table-item__content_${props.item.type}`"
		>
			<AppCheckbox
