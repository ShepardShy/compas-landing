<template>
	...
	<div
		class="warning__text_error"
		v-if="activePayment.value > props.balance && userStore.activePaymentOption && userStore.activePaymentOption.slug == 'compas_pay'"
	>
	...
	<div class="warning__actions">
		<div class="button-details">
			<AppButton
				class="button_blue"
				:disabledOption="userStore.activePaymentOption && userStore.activePaymentOption.slug == 'compas_pay' && activePayment.value > props.balance"
				:class="props.loaderButton ? 'button_loading' : ''"
				@click="() => emit('callAction', { action: 'payment', value: activeOption })"
			>
		...
		<NuxtLink
			to="/settings/?tab=tariffs"
			v-if="activePayment.value > props.balance && userStore.activePaymentOption && userStore.activePaymentOption.slug == 'compas_pay'"
		>
			<AppButton class="button_blue"> Пополнить счет </AppButton>
		</NuxtLink>
		<AppButton @click="() => showWarning(false)"> Отмена </AppButton>
	</div>
</template>
...
<script setup>
	...
	const setValue = computed(() => {
		if (activeOption.value.percent > 0) {
			return ((activePayment.value.value * 100) / (100 - activeOption.value.percent)).toFixed(2);
		} else {
