<template>
	<div
		...
		:class="props.isHaveCategories ? 'table-container_categories' : ''"
	>
		<TableCategory
		...
		<AppSection
			...
			:class="(socketRows.header.length > 0 || socketRows.body.length > 0 ? 'table-template_updating' : '', props.table.loaderState == 'loading' ? 'table-template_loading' : props.table.loaderState == 'filtering' ? 'table-template_filtering' : '')"
		>
			<TableTop
			...
			<div
				...
				:class="[fields.length == 0 || bodyData.length == 0 ? 'table-template__body_empty' : '']"
			>
				<table
					class="table"
					ref="tableRef"
					:class="[props.isPermanentEdit ? 'table_permanent-edit' : '']"
				>
					<TableHeader
			...
			<TableFooter @callAction="data => callAction(data)" />
			...
			<TableWarning @callAction="data => callAction(data)" />
		</AppSection>
	</div>
</template>
