<template>
	<AppWarning
		...
		:isShow="isShow.state"
	>
		<template #title> Ошибка валидации </template>

		<template #body>
			<div class="warning__text">Необходимо заполнить обязательные поля</div>
			<div class="warning__list warning-list">
				<div
					class="warning-list__field"
					v-for="item in invalidFields"
				>
					<AppRelation
					...
					/>
					<AppInput
						v-else-if="['number', 'password', 'email'].includes(item.field.type) || (item.field.type == 'text' && !item.field.is_plural)"
						:item="{
							key: item.field.key,
							type: item.field.type,
							id: item.field.id,
