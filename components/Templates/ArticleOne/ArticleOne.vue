<template>
	...
	<div v-if="articleDetail" class="article" ref="$articleWrapper">
		<div class="article__left">
			<Header :title="name?.value" :image="detail_picture.value?.[0]?.file" :authorAvatar="author?.avatar"
				:authorName="author?.name" :authorDesc="author?.desc" :authorColor="author?.color"
				:date="created_at.value" :update="updated_at.value" :views="views?.value"
			...
			<div class="article__content" ref="$articleContent">
				<template v-for="{
					...
					date,
					readingTime,
				} in detail_text.value">
					<component v-if="type == 'question'" :is="conmponentsMap?.[type]" :text="body"
						:title="questionById(questionId)?.name" :items
						:image="questionById(questionId)?.detail_picture?.[0]?.file"
						:answer="questionById(questionId)?.detail_text?.[0].body"
						:views="questionById(questionId)?.views" :id="questionById(questionId)?.slug"
						:date="questionById(questionId)?.created_at" :type :isShowMore="true" />
					<component v-else :is="conmponentsMap?.[type]" :text="body" :type :title :isShowMore="true" />
				</template>
			</div>
			<!-- </ClientOnly> -->
		</div>
	...
	<Articles v-if="category_id" :categoryId="category_id?.value?.value?.[0]" />
...
await useAsyncData(
	"article",
	async () => await articlesStore.loadArticle(route.params.id)
);
...
detail_text.value = detail_text.value.filter((i) => !i.type.includes('check'));
...
useHead({
	title: `${seo_title.value.value ? seo_title.value.value : seo_title.value
		} | Статьи | Compas.pro`,
	meta: [
		{
			name: "description",
			content: seo_description.value.value
				? seo_description.value.value
